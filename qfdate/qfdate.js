...
function QfDate(){
  var _this=this;
  var qfdateMain,qfdateMask,qfdateCancelBtn,qfdateOkBtn,qfdateYearScroll,qfdateMonthScroll,qfdateDayScroll,qfdateHourScroll,qfdateMinuteScroll;
  var startYear=1900;//起始年份
  var curOpt=null;
  var yData=[],mData=[],dData=[],hData=[],miData=[];
  ...
  this.init=function(){
    $('body').append(
      ...
      '<div class="qfscroll qfdateHourScroll"></div>'+
      '<div class="qfscroll qfdateMinuteScroll"></div>'+
    	'</div>'+
    	'</div>'
    	);
      ...
      qfdateHourScroll=new QfScroll($('.qfdateHourScroll'));
      qfdateMinuteScroll=new QfScroll($('.qfdateMinuteScroll'));
      qfdateYearScroll.init();
      qfdateMonthScroll.init();
      qfdateDayScroll.init();
      qfdateHourScroll.init();
      qfdateMinuteScroll.init();
  ...
  this.getDate=function(){
      return _this.getY()+'-'+_this.getM()+'-'+_this.getD()+' '+_this.getH()+':'+_this.getMi();
  };
  this.getY=function(){
  ...
  this.getH=function(){
      return qfdateHourScroll.getVal();
  };
   this.getMi=function(){
      return qfdateMinuteScroll.getVal();
  };
  ...
  this.showView=function(){
      ...
      var nowH=now.getHours()-1;
      var nowMi=now.getMinutes()-1;
      ...
      qfdateHourScroll.setData(hData,nowH);


      for (var i = 0; i < 60; i++) {
        miData[i]=i+1;
      };
      qfdateMinuteScroll.setData(miData,nowMi);

      var inputValue=curOpt.val();
      // if (isdate(inputValue)) {
      //       var timeValues=inputValue.split(' ');
      //       var dateValues=timeValues[0].split('-');
      //       var dayValues=timeValues[1].split(':');
      ...
      //       qfdateHourScroll.setData(hData,dayValues[0]-1);
      //       qfdateMinuteScroll.setData(miData,dayValues[0]-1);
      // }
    	qfdateMain.show();
  };
};
